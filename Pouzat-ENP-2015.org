...
#+NAME: rss0-definition-and-value-at-initial-guess
#+BEGIN_SRC python :session *ENP2015* :exports none 
...
h0n = hess_num(rss0,np.log(np.copy(par_0)))
not_zero = np.abs(h0) > 0.
np.max(np.abs(h0n[not_zero]-h0[not_zero])/np.abs(h0[not_zero]))
#+END_SRC

#+RESULTS: rss0-definition-and-value-at-initial-guess
: 17547575.915

#+BEGIN_SRC python :session *ENP2015* :exports none 
from scipy.optimize import minimize
ajuste0 = minimize(rss0,np.log(par_0),method='BFGS',options={'disp': True})
#+END_SRC
