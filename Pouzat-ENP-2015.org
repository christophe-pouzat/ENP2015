...
| CCD_calibration.hdf5 | <http.client.HTTPMessage | object | at | 0x7fe62be95b00> |
...
** CCD calibration (9) 						     :export:

+ If the $Y_{i,j,k}$ are not IID we expect a more or less linear trend---due to bleaching of the dye.
+ Rather then looking at each individual pixel sequence like on the previous slide, we can fit the following linear model model to each pixel: $$Y_{i,j,k} = \beta_0 + \beta_1 k + \sigma \epsilon_{i,j}$$ where the $\epsilon_{i,j} \stackrel{IID}{\sim} \mathcal{N}(0,1)$, and check if $\beta_1$ can be reasonably considered as null; will a trend due to bleaching would give a negative $\beta_1$.
...
[(n, int(1000*np.max(np.abs(np.arange(1,len(b1s)+1)/len(b1s)-t.cdf(b1s,98))))/1000) for n, b1s in zip(list(calibration),[np.sort(b1.flatten()) for b1 in b1stats])]
#+END_SRC
