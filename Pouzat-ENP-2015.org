...
P_matrix = np.solve(np.dot(np.transpose(D_matrix),D_matrix),np.transpose(D_matrix))
...
#+BEGIN_SRC python :session *ENP2015* :exports results :results file
plt.figure(dpi=600,figsize=(10,8))
plt.plot(X[:,1],Y,lw=2,color='black')
plt.grid()
plt.xlabel("Time (1 unit = 100 ms)",fontsize=25)
plt.ylabel("ADU",fontsize=25)
plt.plot(X[:,1],Y_hat,lw=2,color='red')
...
+ Using a first order Taylor expansion we then have:\[ \begin{array}{lcl} Z & \approx & f(\mu_Y + \sigma_Y \, \epsilon_2) \\ & \approx & f(\mu_Y) + \sigma_Y \, \epsilon \, \frac{d f}{d Y}(\mu_Y) \end{array}\]
...
We start by defining a symbolic "unitary" rss:
...
#+BEGIN_SRC python :session *ENP2015* :exports none :results output
...
print(rss0(np.log(par_0)))
print(np.max(np.abs(g0-g0n)/np.abs(g0)))
print(np.max(np.abs(h0n[not_zero]-h0[not_zero])/np.abs(h0[not_zero])))
#+END_SRC

#+RESULTS: values-at-initial-guess-and-tests
: 
: 
: 
: 
...
:          Current function value: 1975.519030
:          Iterations: 592
:          Function evaluations: 954
:          Gradient evaluations: 1545
:          Hessian evaluations: 592
...
f_hat = np.zeros((168,))
f_hat[5:] = res.x[13:]
hOn = hess_num(rss0,np.copy(res.x),delta=1e-3)
par_se = np.sqrt(np.diag(scipy.linalg.inv(hOn)))
